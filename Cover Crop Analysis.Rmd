...
```{r weed count analysis -- model selection}
# weed count models
...
```{r weed count analysis -- assumptions & means}
model.weedcount.best<- glmer(Count ~ Cult * SR * Time + (1 | Year / Rep) + (1 | Location / Rep) + (1 | Year:Location:Plot), data=wdat, family=poisson(link="log"))
Anova(model.weedcount.best, type="III")
...
#  means
wcountemmeans<- emmeans(model.weedcount.best, pairwise ~ Cult*SR*Time, type="response")
wcountemmeans
plot(wcountemmeans)
```
*Best Model:*
Count ~ Cult * SR * Time + (1 | Year / Rep) + (1 | Location / Rep) + (1 | Year:Location:Plot)

*Effects:*
             npar  Sum Sq Mean Sq   F value
Cult            1    3.02    3.02    3.0199
SR              2    3.30    1.65    1.6517
Time            2 3062.51 1531.25 1531.2546
Cult:SR         2    0.09    0.04    0.0435
Cult:Time       2   34.18   17.09   17.0881
SR:Time         4   17.92    4.48    4.4808
Cult:SR:Time    4   51.41   12.85   12.8536

                Chisq Df Pr(>Chisq)    
(Intercept)   15.7688  1  7.157e-05 ***
Cult           0.9423  1     0.3317    
SR             2.7427  2     0.2538    
Time         677.0624  2  < 2.2e-16 ***
Cult:SR        1.1116  2     0.5736    
Cult:Time     31.6768  2  1.323e-07 ***
SR:Time       23.9742  4  8.083e-05 ***
Cult:SR:Time  51.5960  4  1.676e-10 ***

*Assumptions:*
Bad

*Means:*
 Cult     SR  Time   rate    SE  df asymp.LCL asymp.UCL
 Early    150 28DAT  4.62 1.783 Inf     2.172      9.85
 Standard 150 28DAT  3.73 1.428 Inf     1.763      7.90
 Early    300 28DAT  3.57 1.381 Inf     1.675      7.62
 Standard 300 28DAT  2.12 0.817 Inf     0.996      4.51
 Early    600 28DAT  3.19 1.236 Inf     1.496      6.82
 Standard 600 28DAT  2.47 0.946 Inf     1.169      5.23
 Early    150 56DAT  8.21 3.161 Inf     3.861     17.46
 Standard 150 56DAT  8.25 3.149 Inf     3.904     17.43
 Early    300 56DAT  5.97 2.301 Inf     2.801     12.70
 Standard 300 56DAT  6.04 2.308 Inf     2.856     12.77
 Early    600 56DAT  7.20 2.774 Inf     3.380     15.32
 Standard 600 56DAT  5.13 1.952 Inf     2.433     10.81
 Early    150 PRE   12.77 4.909 Inf     6.008     27.12
 Standard 150 PRE    9.92 3.785 Inf     4.700     20.96
 Early    300 PRE   10.31 3.967 Inf     4.847     21.91
 Standard 300 PRE    7.74 2.955 Inf     3.666     16.36
 Early    600 PRE    9.94 3.825 Inf     4.673     21.13
 Standard 600 PRE    8.09 3.074 Inf     3.844     17.04
...
```{r model corn ht orientation}
model.hto<- lmer(Corn_ht_av ~ Orientation * Weediness + (1 | Year/Rep) + (1 | Location/Rep), data=dirtrt)
summary(model.hto)
anova(model.hto)

## model reduction
model.hto.best<- get_model(lmerTest::step(model.hto, reduce.random=F))
summary(model.hto.best)
anova(model.hto.best)

# check residuals
resid.model.hto<- resid(model.hto.best)
pred.model.hto<- predict(model.hto.best)

# check randomness
plot(pred.model.hto, resid.model.hto, ylab="Residuals", xlab="Corn height", main="Predictecd vs Residuals")

# check normality
hist(resid.model.hto, main="Histogram of Residuals", xlab="Corn height")

qqnorm(resid.model.hto) 

# Shapiro-Wilk test
shapiro.test(resid.model.hto)
# test assumptions good
```
*Best Model:*
Corn_ht_av ~ (1 | Year/Rep) + (1 | Location/Rep)
...
model.inj1<- glmer(injury ~ Trt_num * Weediness + (1 | Year/Rep) + (1 | Location/Rep) + (1 | Plot), data=dirinj, family=poisson(link="log"))
summary(model.inj1) # AIC: 6883.6

model.inj2<- glmer(injury ~ Trt_num + (1 | Year/Rep) + (1 | Location/Rep) + (1 | Plot), data=dirinj, family=poisson(link="log"))
summary(model.inj2) # AIC: 6930.4

model.inj3<- glmer(injury ~  Weediness + (1 | Year/Rep) + (1 | Location/Rep) + (1 | Plot), data=dirinj, family=poisson(link="log"))
summary(model.inj3) # AIC: 6988.6

model.inj4<- glmer(injury ~ 1 + (1 | Year/Rep) + (1 | Location/Rep) + (1 | Plot), data=dirinj, family=poisson(link="log"))
summary(model.inj4) # AIC: 6994.8

model.inj5<- glmer(injury ~ Trt_num + Weediness + (1 | Year/Rep) + (1 | Location/Rep) + (1 | Plot), data=dirinj, family=poisson(link="log"))
summary(model.inj5) # AIC: 6924.2

model.inj.best<- glmer(injury ~ Trt_num * Weediness + (1 | Year/Rep) + (1 | Location/Rep) + (1 | Plot), data=dirinj, family=poisson(link="log"))
...
```
*Best Model:*
injury ~ Trt_num * Weediness + (1 | Year/Rep) + (1 | Location/Rep) + (1 | Plot)

*Effects:*
                   Chisq Df Pr(>Chisq)    
(Intercept)       45.541  1  1.495e-11 ***
Trt_num           97.091  5  < 2.2e-16 ***
Weediness         15.812  1  6.997e-05 ***
Trt_num:Weediness 50.460  5  1.116e-09 ***
...
*Means:*
 ...
 1       W           3.51 0.52 Inf      2.49      4.53
 2       W           3.68 0.52 Inf      2.66      4.70
 3       W           3.27 0.52 Inf      2.25      4.29
 4       W           3.65 0.52 Inf      2.63      4.67
 5       W           3.42 0.52 Inf      2.40      4.44
 6       W           3.33 0.52 Inf      2.31      4.35
 1       WF          3.35 0.52 Inf      2.33      4.37
 2       WF          3.48 0.52 Inf      2.47      4.50
 3       WF          3.43 0.52 Inf      2.41      4.45
 4       WF          3.59 0.52 Inf      2.57      4.61
 5       WF          3.44 0.52 Inf      2.42      4.46
 6       WF          3.33 0.52 Inf      2.31      4.34


```{r model corn injury trt level, results='hide', message=FALSE, warning=FALSE}
model.injtrt1<- glmer(injury ~ Trt_crmp * Trt_plnt * Weediness + (1 | Year/Rep) + (1 | Location/Rep) + (1 | Plot), data=dirinjtrt, family=poisson(link="log"))
summary(model.injtrt1) # AIC: 2327.6

model.injtrt2<- glmer(injury ~ Trt_crmp * Trt_plnt + (1 | Year/Rep) + (1 | Location/Rep) + (1 | Plot), data=dirinjtrt, family=poisson(link="log"))
summary(model.injtrt2) # AIC: 2336.1

model.injtrt3<- glmer(injury ~ Trt_crmp * Weediness + (1 | Year/Rep) + (1 | Location/Rep) + (1 | Plot), data=dirinjtrt, family=poisson(link="log"))
summary(model.injtrt3) # AIC: 6970.9

model.injtrt4<- glmer(injury ~ Trt_plnt * Weediness + (1 | Year/Rep) + (1 | Location/Rep) + (1 | Plot), data=dirinjtrt, family=poisson(link="log"))
summary(model.injtrt4) # AIC: 2360.1

model.injtrt5<- glmer(injury ~ Trt_crmp + Trt_plnt + Weediness + (1 | Year/Rep) + (1 | Location/Rep) + (1 | Plot), data=dirinjtrt, family=poisson(link="log"))
summary(model.injtrt5) # AIC: 2360.9

model.injtrt6<- glmer(injury ~ Trt_crmp * Trt_plnt + Weediness + (1 | Year/Rep) + (1 | Location/Rep) + (1 | Plot), data=dirinjtrt, family=poisson(link="log"))
summary(model.injtrt6) # AIC: 2336.5

model.injtrt7<- glmer(injury ~ Trt_crmp + Trt_plnt * Weediness + (1 | Year/Rep) + (1 | Location/Rep) + (1 | Plot), data=dirinjtrt, family=poisson(link="log"))
summary(model.injtrt7) # AIC: 2362.1

model.injtrt8<- glmer(injury ~ Trt_crmp * Weediness + Trt_plnt + (1 | Year/Rep) + (1 | Location/Rep) + (1 | Plot), data=dirinjtrt, family=poisson(link="log"))
summary(model.injtrt8) # AIC: 2354.6

model.injtrt.best<- glmer(injury ~ Trt_crmp * Trt_plnt * Weediness + (1 | Year/Rep) + (1 | Location/Rep) + (1 | Plot), data=dirinjtrt, family=poisson(link="log"))
...
```
*Best Model:*
injury ~ Trt_crmp + Trt_plnt + Weediness + (1 | Year/Rep) + (1 | Location/Rep) + (1 | Plot) + Trt_crmp : Trt_plnt : Weediness + Trt_crmp : Trt_plnt + Trt_crmp : Weediness + Trt_plnt : Weediness
...
*Means:*
 ...
 E        E        W         29.8 18.2 Inf      9.04      98.5
 N        E        W         36.2 22.1 Inf     10.96     119.5
 E        N        W         38.0 23.1 Inf     11.51     125.4
 N        N        W         28.3 17.2 Inf      8.56      93.3
 E        E        WF        29.7 18.1 Inf      8.99      98.0
 N        E        WF        36.9 22.5 Inf     11.19     122.0
 E        N        WF        35.9 21.9 Inf     10.88     118.5
 N        N        WF        33.3 20.3 Inf     10.09     109.9
...
wtab2<-subset(dirw, Time=="56DAT") %>%
  group_by(Location, Year, Plot, Rep, Trt_num, Trt_plnt, Trt_crmp) %>%
