...
datwf<- dat # datwf contains all trts, including WF rye

dat<- subset(dat, Trt_num<9) # remove WF rye trts

datr<- subset(dat, Cult!="NA") # datr contains only trt plots
...
ryewtplot1<- ggplot(subset(dat, Cult=="Standard"|Cult=="Early"), aes(x = SR, y = Rye_wt, fill=Cult)) +
          geom_boxplot() +
          theme_bw()
print(ryewtplot1)
...
model.ryewt<- lmer(Rye_wt ~ Cult * SR * Location + ( 1 | Rep:Year), data=dat)
summary(model.ryewt)
anova(model.ryewt)
...
# (Cult not signif in model when WF no rye plots removed)
ryewtemmeans3<- emmeans(model.ryewt.best, pairwise ~ Cult)
ryewtemmeans3 # Early greater than standard
plot(ryewtemmeans3, comparisons=T)
```
...
tillplot1<- ggplot(subset(dat, Cult=="Standard"|Cult=="Early"), aes(x = Cult, y = TAv, fill=factor(SR))) +
          geom_boxplot() +
          facet_wrap(Year~Location)+
          theme_bw()
print(tillplot1)
...
model.till<- lmer(TAv ~ Cult * SR * Location + ( 1 | Rep:Year), data=dat)
summary(model.till)
anova(model.till)
...
max(dat$TAv, na.rm=T)
Toutlier.rm<- dat[-c(grep("14", dat$TAv)),]
...
diamdat<- subset(dat, Location=="Harrow") # subset data to only include Harrow b/c diameter only measured in Harrow
...
standplot1<- ggplot(dat, aes(x = Trt_code, y = Corn_stand, fill=factor(SR))) +
          geom_boxplot() +
...
# test effect of rye
model.stand<- lmer(Corn_stand ~ Trt_code * Location + ( 1 | Rep:Year), data=dat)
summary(model.stand)
anova(model.stand)
...
# test effect of trts
model.standtrt<- lmer(Corn_stand ~ Cult * SR * Location + ( 1 | Rep:Year), data=datr)
...
model.standtrt2<- lmer((Corn_stand)^1/3 ~ Cult * SR * Location + ( 1 | Rep:Year), data=datr)
...
htplot1<- ggplot(dat, aes(x = Trt_code, y = Corn_ht_av, fill=factor(SR))) +
          geom_boxplot() +
          facet_wrap(Location~Year)+
          theme_bw()
print(htplot1)
...
# model analysis for average corn height

# test effect of trts vs ctrls
model.ht<- lmer(Corn_ht_av ~ Trt_code * Location + ( 1 | Rep:Year), data=dat)
summary(model.ht)
anova(model.ht)

htemmeans<- emmeans(model.ht, pairwise ~ Trt_code|Location)
htemmeans
plot(htemmeans, comparisons=T) # crtls higher than most trts in StJean & Harrow, not in Agassiz
...
# trt effect
model.ht.trt<- lmer(Corn_ht_av ~ Cult * SR * Location + ( 1 | Rep:Year), data=datr)
...
```{r corn ht analysis -- w vs wf trts}
...
Mplot1<- ggplot(dat, aes(x = Trt_code, y = M_count, fill=factor(SR))) +
          geom_boxplot() +
          facet_wrap(Location~Year)+
          theme_bw()
print(Mplot1)
...
model.M<- lmer(M_count ~ Trt_code * Location + ( 1 | Rep:Year), data=dat)
summary(model.M)
anova(model.M)
...
model.M.trt<- lmer(M_count ~ Cult * SR * Location + ( 1 | Rep:Year), data=datr)
summary(model.M.trt)
anova(model.M.trt)
...
```{r M corn count analysis -- w vs wf trts}
...
Mwtplot1<- ggplot(dat, aes(x = Trt_code, y = M_wt, fill=factor(SR))) +
          geom_boxplot() +
          facet_wrap(Location~Year)+
          theme_bw()
print(Mwtplot1)
...
model.Mwt<- lmer(M_wt ~ Trt_code * Location + ( 1 | Rep:Year), data=dat)
summary(model.Mwt)
anova(model.Mwt)
...
model.Mwt.trt<- lmer(M_wt ~ Cult * SR * Location + ( 1 | Rep:Year), data=datr)
...
UMplot1<- ggplot(dat, aes(x = Trt_code, y = UM_count, fill=factor(SR))) +
          geom_boxplot() +
          facet_wrap(Location~Year)+
          theme_bw()
print(UMplot1)
...
model.UM<- lmer(UM_count ~ Trt_code * Location + ( 1 | Rep:Year), data=dat)
summary(model.UM)
anova(model.UM)
...
model.UM.trt<- lmer(UM_count ~ Cult * SR * Location + ( 1 | Rep:Year), data=datr)
...
```{r UM corn count analysis -- w vs wf}
...
UMwtplot1<- ggplot(dat, aes(x = Trt_code, y = UM_wt, fill=factor(SR))) +
          geom_boxplot() +
          facet_wrap(Location~Year)+
          theme_bw()
print(UMwtplot1)
...
model.UMwt<- lmer(UM_wt ~ Trt_code * Location + ( 1 | Rep:Year), data=dat)
summary(model.UMwt)
anova(model.UMwt)
...
# model analysis for unmarketable cob weight trt vs ctrl
model.UMwt.trt<- lmer(UM_wt ~ Cult * SR * Location + ( 1 | Rep:Year), data=datr)
...
```{r UM corn wt analysis -- w vs wf}

```


### Corn analysis with W/WF subfactor

```{r}
datW<- subset(dat, Location=="Harrow" & Year=="2020"|Year=="2021")
str(datW)
datW$Weediness
```

None of these models are working, most say Error: number of rows in use has changed: remove missing values? But there shouldn't be any missing values 

#### Corn stand

```{r}
# plot of corn stand by cultivar & seeding rate
wstandplot1<- ggplot(datW, aes(x = Trt_code, y = Corn_stand, fill=factor(Weediness))) +
          geom_boxplot() +
          theme_bw()
print(wstandplot1)

# model analysis for corn stand (count data so glmer used with poisson distribution)
model.standw<- glmer(Corn_stand ~ Cult * SR * Weediness + ( 1 | Rep), data=datW, family=poisson) # suggests model may be singular
summary(model.standw)
Anova(model.standw, type="III")

isSingular(model.standw) # True

model.standw.best<- get_model(lmerTest::step(model.standw, reduce.random=F))
model.standw.best<- glmer(Corn_stand ~ 1 + (1 | Rep), data=datW, family=poisson)
...
# data assumptions not met. data mostly singular

emmeans(model.standw, pairwise ~ Weediness)
plot(emmeans(model.standw, pairwise ~ Weediness))
```

#### corn height
Having issues with lmerTest::step saying change in variable length. Anova of full model indicates Weediness is significant -> weedy plots have taller height
```{r}
# plot of average corn height by cultivar & seeding rate
htplotw<- ggplot(datW, aes(x = Trt_code, y = Corn_ht_av, fill=factor(Weediness))) +
          geom_boxplot() +
          theme_bw()
print(htplotw)

dataW.na.omit<- datW[!is.na(datW$Corn_ht_av),]

# model analysis for average corn height
model.htw<- lmer(Corn_ht_av ~ Cult * SR * Weediness + ( 1 | Rep), data=dataW.na.omit)
summary(model.htw)
anova(model.htw)
...
htwemmeans<- emmeans(model.htw, pairwise ~ Weediness)
htwemmeans # weedy plots taller by ~ 3cm
```

#### Marketable corn count

```{r}
# plot of marketable cob count by cultivar & seeding rate
Mplotw<- ggplot(datw, aes(x = Trt_code, y = M_count, fill=factor(Weediness))) +
          geom_boxplot() +
          theme_bw()
print(Mplotw)

# model analysis for marketable corn count
model.Mw<- lmer(M_count ~ Cult * SR * Weediness + ( 1 | Rep), data=datW)
summary(model.Mw)
anova(model.Mw)

isSingular(model.Mw) # True

model.Mw.best<- get_model(lmerTest::step(model.Mw, reduce.random=F)) ## Not working -- number of rows in use has changed: remove missing values?
summary(model.M.best)
anova(model.M.best)

Mwemmeans<- emmeans(model.Mw, pairwise ~ Weediness)
Mwemmeans # WF plots have about 4 more marketable cobs than W
```

#### Marketable corn weight

```{r}
# plot of marketable cob weight by cultivar & seeding rate
Mwtplotw<- ggplot(datW, aes(x = Trt_code, y = M_wt, fill=factor(Weediness))) +
          geom_boxplot() +
          theme_bw()
print(Mwtplotw)

# model analysis for marketable cob weight
model.Mwtw<- lmer(M_wt ~ Cult * SR * Weediness + ( 1 | Rep), data=datW)
summary(model.Mwtw)
anova(model.Mwtw)

model.Mwtw.best<- get_model(lmerTest::step(model.Mwtw, reduce.random=F)) # Not working -> number of rows in use has changed
summary(model.Mwt.best)
anova(model.Mwt.best)

Mwtwemmeans<- emmeans(model.Mwtw, pairwise ~ Weediness)
Mwtwemmeans # WF has 1945g more marketable weight than W
Mwtemmeans2<- emmeans(model.Mwtw, pairwise ~ Cult)
Mwt.emmeans2
Mwtemmeans3<- emmeans(model.Mwtw, pairwise ~ Weediness|Cult)
Mwtemmeans3
```
...
```{r}
# plot of unmarketable cob count by cultivar & seeding rate
UMplotw<- ggplot(datW, aes(x = Trt_code, y = UM_count, fill=factor(Weediness))) +
          geom_boxplot() +
          theme_bw()
print(UMplotw)

# model analysis for unmarketable cob count
model.UMw<- lmer(UM_count ~ Cult * SR * Weediness + ( 1 | Rep), data=datW)
summary(model.UMw)
anova(model.UMw)

model.UMw.best<- get_model(lmerTest::step(model.UMw, reduce.random=F)) # Not working -> number of rows in use has changed
...
```{r}
# plot of unmarketable cob weight by cultivar & seeding rate
UMwtplotw<- ggplot(datW, aes(x = Trt_code, y = UM_wt, fill=factor(Weediness))) +
          geom_boxplot() +
          theme_bw()
print(UMwtplotw)

# model analysis for unmarketable cob weight
model.UMwtw<- lmer(UM_wt ~ Cult * SR * Weediness + ( 1 | Rep), data=datW)
summary(model.UMwtw)
anova(model.UMwtw)

model.UMwtw.best<- get_model(lmerTest::step(model.UMwtw, reduce.random=F)) # Not working -> number of rows in use has changed
